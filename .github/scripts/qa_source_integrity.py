...
check('child updates require Jira update events', 'if(!isUpdated)return' in engine)
check('explicit child outcome mapper retained', 'taskOutcomeFromStatus' in engine)
check('Accepted can route before Done', "['accepted',['accepted','accept']]" in engine)
check('Done fallback only maps safe completion outcomes', "if(cat==='done')" in engine and "available.includes('completed')" in engine)
