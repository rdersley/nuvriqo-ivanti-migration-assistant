...
function updateServiceFields(project:Project,serviceId:string,fieldResults:any[]):Project{ const now=new Date().toISOString(); return {...project,updatedAt:now,services:(project.services||[]).map(service=>service.id===serviceId?{...service,updatedAt:now,build:{...(service.build||{}),fieldResults}}:service)}; }
...
function mappedFormFields(service:Service,fieldResults:any[]){ const byName=new Map(fieldResults.filter(r=>r?.id).map(r=>[clean(r.name).toLowerCase(),r])); return (service.analysis?.fields||[]).map(f=>({sourceId:f.id,id:(byName.get(clean(f.name).toLowerCase()) as any)?.id,name:f.name,required:Boolean(f.required),jiraType:f.jiraType,options:f.options||[]})); }
...
export default function FullMigrationRunnerMount(){
  ...
  useEffect(()=>{
    ...
    const find=()=>{
      ...
      const wizard=panels.find(p=>p.querySelector('h1')?.textContent?.trim()==='Migration wizard');
      setTarget(wizard||null);
    };
    refresh(); find();
  ...
  return createPortal(<section style={{marginTop:24,border:'2px solid #0c66e4',borderRadius:8,padding:20,background:'#fff'}} data-full-migration-runner="true">
    ...
    <p style={{margin:0,color:'#626f86'}}>This is the live captured-service runner. It creates/reuses fields, Jira issue type/workflow structure, JSM request type and Form, then verifies portal visibility. The project workflow scheme is not replaced automatically until all service mappings can be consolidated safely.</p>
    ...
    {!ready.length&&<p style={{marginTop:10,color:'#974f0c'}}>No mapped runtime workflow is stored on a service yet. Your existing GetInstance capture should remain saved; return to Settings only if this message persists after refresh.</p>}
