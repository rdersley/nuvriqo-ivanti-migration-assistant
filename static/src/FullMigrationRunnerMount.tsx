...
export default function FullMigrationRunnerMount(){
  ...
  useEffect(()=>{
    ...
    const find=()=>{
      ...
      setTarget(panels.find(p=>p.querySelector('h1')?.textContent?.trim()==='Migration wizard')||null);
    };
  ...
  async function run(){
    ...
    if(!window.confirm(`Repair/preserve the existing form and add the captured workflow for ${ready.length} mapped service${ready.length===1?'':'s'}?`))return;
    ...
    for(const service of ready){
      ...
      try{
        ...
        if(unresolved.length) throw new Error(`Refusing to rebuild the form with missing Jira field mappings: ${unresolved.map(f=>f.name).join(', ')}`);
        const form:any=await invoke('createJsmForm',{projectId:working.targetProjectId,requestTypeId:request.requestTypeId,name:`${name} - Ivanti Migration Form`,fields:formFields,sections:service.formSections||[],conditions:service.proposedConditions||[]});
        if(!form?.published) throw new Error(form?.message||'JSM Form was not published.');
        steps.push({key:'form',status:'ok',message:`Complete ${formFields.length}-field form ${form.formId||'created'} published.`});
  ...
  return createPortal(<section style={{marginTop:24,border:'2px solid #0c66e4',borderRadius:8,padding:20,background:'#fff'}} data-full-migration-runner="true">
    <div style={{fontSize:12,fontWeight:700,letterSpacing:'.08em',color:'#44546f',textTransform:'uppercase'}}>Live migration execution</div>
    <h2 style={{margin:'5px 0 4px'}}>Preserve form + add captured workflow</h2>
    <p style={{margin:0,color:'#626f86'}}>The workflow migration no longer rebuilds a form from only the latest field-creation results. It resolves every existing Jira field first and refuses to publish if any source field is unmapped.</p>
    <div style={{marginTop:14,display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}><button disabled={busy||!project.targetProjectId||!ready.length} onClick={()=>void run()}>{busy?'Repairing and building…':`Repair form and build ${ready.length} captured service${ready.length===1?'':'s'}`}</button><span style={{fontSize:13,color:'#626f86'}}>{project.targetProjectId?`Target project ${project.targetProjectId}`:'Choose a target project first.'}</span></div>
