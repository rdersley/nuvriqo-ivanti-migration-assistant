...
type FormIndex = { id: string; name: string; portalRequestTypeIds?: number[] };
type FormDesign = {
...
function removeQuestionFromLayout(nodes: any[], questionId: string): any[] {
  const visit = (node: any): any | null => {
    ...
    if (node.type === 'extension' && attrs?.extensionKey === 'question' && String(attrs?.parameters?.id ?? '') === String(questionId)) {
      return null;
    }
    if (Array.isArray(node.content)) {
      node = { ...node, content: node.content.map(visit).filter((item: any) => item !== null) };
    }
    return node;
  };
...
async function ensureTextField(catalogue: JiraField[], label: string): Promise<JiraField> {
  ...
  const existing = catalogue.find((field) =>
    normalise(field.name) === normalise(repairName) &&
    String(field.schema?.custom ?? '').includes('customfieldtypes:textfield')
  );
...
export async function repairMigratedForms(projectId: string, serviceNames: string[]) {
  ...
  const wanted = new Set(serviceNames.map(normalise).filter(Boolean));
  ...
  const targets = forms.filter((form) => wanted.has(normalise(form.name)));
  ...
  for (const form of targets) {
    ...
    const stored = await parseResponse<{ design?: FormDesign }>(getResponse);
    ...
    for (const [questionId, question] of Object.entries(questions)) {
      ...
      // Ivanti manager lookups are business-object lookups, not necessarily Jira users.
      // A Jira select/user picker with no matching catalogue makes the migrated request impossible to submit.
    ...
    if (changed) {
      ...
      const requestTypeIds = (form.portalRequestTypeIds ?? []).map(Number).filter(Number.isFinite);
      const body: any = { design };
      if (requestTypeIds.length) {
        body.publish = {
          jira: { issueCreateIssueTypeIds: [], issueCreateRequestTypeIds: requestTypeIds, recommendedIssueRequestTypeIds: [], submitOnCreate: true, validateOnCreate: true },
          portal: { portalRequestTypeIds: requestTypeIds, submitOnCreate: true, validateOnCreate: true }
        };
      }
    ...
    results.push({ formId: String(form.id), name: form.name, repairedLookups, hiddenHelpers, changed });
  }

  return { projectId, matchedForms: targets.length, changedForms: results.filter((item) => item.changed).length, results };
}
